...
use Email::MIME;
use Data::Dumper;
use PAUSE::Web2025::Config;
use PAUSE::Web2025::Exception;
use Auth::GoogleAuth;

our $VERSION = "1072";

has root => sub { Carp::confess "requires root" };
has config => sub { PAUSE::Web2025::Config->new };
...
sub database_alert {
  ...
  die PAUSE::Web2025::Exception->new(ERROR => <<"ERROR_END");
...
sub send_mail {
  ...
  if (my $error = $@) {
    if ($error->isa('Email::Sender::Failure')) {
      ...
      die PAUSE::Web2025::Exception->new(ERROR => Mojo::ByteStream->new(<<"ERROR_END"));
