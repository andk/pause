package PAUSE::Web2025::Controller::Public::RequestId;

use Mojo::Base "Mojolicious::Controller";
use PAUSE::Web2025::Util::Encode;
use Email::Address;
...
sub request {
  ...
  my $ufullname = PAUSE::Web2025::Util::Encode::any2utf8($fullname);
  ...
  my $urat = PAUSE::Web2025::Util::Encode::any2utf8($rationale);
  ...
  if ($regOK) {
    ...
    if (lc($fullname) eq lc($userid)) {
      die PAUSE::Web2025::Exception->new(ERROR => "fullname looks like spam");
    }
    if (my @x = $rationale =~ /(\.info)/g) {
      die PAUSE::Web2025::Exception->new(ERROR => "rationale looks like spam") if @x >= 5;
    }
    if (my @x = $rationale =~ m|(http://)|g) {
      die PAUSE::Web2025::Exception->new(ERROR => "rationale looks like spam") if @x >= 5;
    }
    if ($rationale =~ /interesting/i && $homepage =~ m|http://[^/]+\.cn/.+\.htm$|) {
      die PAUSE::Web2025::Exception->new(ERROR => "rationale looks like spam");
    }
