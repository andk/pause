package PAUSE::Web2025::Controller::Public;
...
sub mailpw {
  ...
  if ( uc $req->method eq 'POST' and $param ) {
    ...
    unless ($param =~ /^[A-Z\-]+$/) {
      if ($param =~ /@/) {
        die PAUSE::Web2025::Exception->new(ERROR =>
    ...
    if ($sth->rows == 1) {
      ...
      if ($@) {
        ...
        $authen_dbh->do($sql,{},$u->{userid},$u->{email},time)
            or die PAUSE::Web2025::Exception->new(ERROR =>
 ...
 is not known, please retry with a valid userid.}); # FIXME
    ...
    if ($email) {
      ...
      if ( $authen_dbh->do($sql,undef,$param,$passwd,$then) ) {
        ...
        die PAUSE::Web2025::Exception->new
            (
        ...
        die PAUSE::Web2025::Exception->new(ERROR => $authen_dbh->errstr);
      }
